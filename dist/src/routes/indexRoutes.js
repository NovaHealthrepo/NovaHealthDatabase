...
router.get("/indices", async (_req, res) => {
    try {
        ...
        if (s13)
            console.log('[BACKEND] Service 13 from Prisma:', { serviceID: s13.serviceID, duration: s13.duration, type: typeof s13.duration });
        res.json(indices);
    }
    catch (err) {
        console.error(err);
...
router.post("/indices", async (req, res) => {
    try {
        const { time, date, duration, userID, staffID, Price, salary, isfreelance, reportTime, isReserved, isAttended, isRecorded, } = req.body;
...
router.put("/indices/:id", async (req, res) => {
    try {
        ...
        const data = {};
        if (time !== undefined)
            data.time = new Date(`1970-01-01T${time}`);
        if (date !== undefined)
...
router.get("/index-users", async (_req, res) => {
    try {
...
router.get("/index-staff", async (_req, res) => {
    try {
...
router.get("/user-price-sum", async (req, res) => {
    try {
...
router.get("/staff-salary-sum", async (req, res) => {
    try {
        ...
        const totalSalary = records.reduce((sum, record) => sum + Number(record.salary), 0);
        const totalHours = records.reduce((sum, record) => sum + Number(record.duration), 0);
        res.json({
            staffID: staffId,
            year: yearNum,
...
router.get("/monthly-summary", async (req, res) => {
    try {
        ...
        if (type === "user") {
            ...
            // Group by staff
            const staffSummary = records.reduce((acc, record) => {
